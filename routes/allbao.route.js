...
router.get('/chitiet/:idbao', async function (req, res) {
    ...
    const rows1 = await baoModel.allbl(req.params.idbao);
...
router.post('/chitiet/:id', async function (req, res){
  ...
  if (baoObject) {
    await baoModel.patch({
      idbao: baoObject.idbao,
      slbinhluan: baoObject.slbinhluan + 1,
    });
  }
  await baoModel.addcm(entity);
  res.redirect(`/bao/chitiet/${req.body.idbao}`);
})
